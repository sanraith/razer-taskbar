...
type MessageEvents = {
  [Property in keyof AppSettings]: (value: AppSettings[Property]) => void;
} & {
  _defaultSettingsCreated: () => void;
};
export const settingsChanges =
  new SettingsEmitter() as TypedEventEmitter<MessageEvents>;
...
export async function updateSettings(changes: Partial<AppSettings>) {
  ...
  Object.entries(changes).map(([k, v]) =>
    settingsChanges.emit(k as keyof AppSettings, v)
  );
  await saveSettings();
}
...
export async function loadSettings() {
  ...
  let settingsString = "";
  try {
    settingsString = await fsa.readFile(SETTINGS_FILE_PATH, {
      encoding: "utf8",
    });
